...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.82.01), Wed Feb  8 14:02:56 2017
...
expName = u'experiment_premade_feedback'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/Rivendell/Documents/UWA/Psychopy_workshop/Example experiment2/experiment_premade_feedback.psyexp',
...
welcome_slide = visual.ImageStim(win=win, name='welcome_slide',
    image='welcome_slide.png', mask=None,
...
error_msg =  "Whoops, forgot to update this!" 
...
practice_loop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=u'/Users/Rivendell/Documents/UWA/Psychopy_workshop/Example experiment2/experiment_premade_feedback.psyexp',
...
for thisPractice_loop in practice_loop:
    ...
    response_keys_practice = event.BuilderKeyResponse()  # create an object of type KeyResponse
    response_keys_practice.status = NOT_STARTED
    # keep track of which components have finished
    ...
    practice_trialComponents.append(response_keys_practice)
    ...
    while continueRoutine:
        ...
        # *response_keys_practice* updates
        if t >= 1.5 and response_keys_practice.status == NOT_STARTED:
            # keep track of start time/frame for later
            response_keys_practice.tStart = t  # underestimates by a little under one frame
            response_keys_practice.frameNStart = frameN  # exact frame index
            response_keys_practice.status = STARTED
            # keyboard checking is just starting
            response_keys_practice.clock.reset()  # now t=0
            event.clearEvents(eventType='keyboard')
        if response_keys_practice.status == STARTED:
            ...
            if "escape" in theseKeys:
                endExpNow = True
            if len(theseKeys) > 0:  # at least one key was pressed
                response_keys_practice.keys = theseKeys[-1]  # just the last key pressed
                response_keys_practice.rt = response_keys_practice.clock.getTime()
                # was this 'correct'?
                if (response_keys_practice.keys == str(correctAns)) or (response_keys_practice.keys == correctAns):
                    response_keys_practice.corr = 1
                else:
                    response_keys_practice.corr = 0
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in practice_trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
    if response_keys_practice.keys in ['', [], None]:  # No response was made
       response_keys_practice.keys=None
       # was no response the correct answer?!
       if str(correctAns).lower() == 'none': response_keys_practice.corr = 1  # correct non-response
       else: response_keys_practice.corr = 0  # failed to respond (incorrectly)
    # store data for practice_loop (TrialHandler)
    practice_loop.addData('response_keys_practice.keys',response_keys_practice.keys)
    practice_loop.addData('response_keys_practice.corr', response_keys_practice.corr)
    if response_keys_practice.keys != None:  # we had a response
        practice_loop.addData('response_keys_practice.rt', response_keys_practice.rt)
    ...
    # update component parameters for each repeat
    #if incorrect show error message, otherwise show positive feedback
    if response_keys_practice.corr == False:
        error_msg = "Oops! That was wrong!"
    elif response_keys_practice.corr == True:
        error_msg = "Correct! \nExcellent animal recognition!"
    
    feedbacktext.setText(error_msg)
    # keep track of which components have finished
    FeedbackComponents = []
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if feedbacktext.status == STARTED and t >= (0.0 + (1.0-win.monitorFramePeriod*0.75)): #most of one frame period left
...
for thisExperiment_loop in experiment_loop:
    ...
    response_keys = event.BuilderKeyResponse()  # create an object of type KeyResponse
    response_keys.status = NOT_STARTED
    # keep track of which components have finished
    trialComponents = []
    trialComponents.append(real_image)
    trialComponents.append(fixation_cross)
    trialComponents.append(response_keys)
    ...
    while continueRoutine:
        ...
        # *response_keys* updates
        if t >= 1.5 and response_keys.status == NOT_STARTED:
            # keep track of start time/frame for later
            response_keys.tStart = t  # underestimates by a little under one frame
            response_keys.frameNStart = frameN  # exact frame index
            response_keys.status = STARTED
            # keyboard checking is just starting
            response_keys.clock.reset()  # now t=0
            event.clearEvents(eventType='keyboard')
        if response_keys.status == STARTED:
            theseKeys = event.getKeys(keyList=['y', 'n'])
            ...
            if "escape" in theseKeys:
                endExpNow = True
            if len(theseKeys) > 0:  # at least one key was pressed
                response_keys.keys = theseKeys[-1]  # just the last key pressed
                response_keys.rt = response_keys.clock.getTime()
                # was this 'correct'?
                if (response_keys.keys == str(correctAns)) or (response_keys.keys == correctAns):
                    response_keys.corr = 1
                else:
                    response_keys.corr = 0
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in trialComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # check responses
    if response_keys.keys in ['', [], None]:  # No response was made
       response_keys.keys=None
       # was no response the correct answer?!
       if str(correctAns).lower() == 'none': response_keys.corr = 1  # correct non-response
       else: response_keys.corr = 0  # failed to respond (incorrectly)
    # store data for experiment_loop (TrialHandler)
    experiment_loop.addData('response_keys.keys',response_keys.keys)
    experiment_loop.addData('response_keys.corr', response_keys.corr)
    if response_keys.keys != None:  # we had a response
        experiment_loop.addData('response_keys.rt', response_keys.rt)
